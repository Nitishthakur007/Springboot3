...
class EcommerceApplicationTests {
	...
	public void dbCategoryDefaultData() throws Exception {
		 ...
		 for(ListIterator<?> it = ((JSONArray) new JSONParser()
					.parse(new ObjectMapper().writeValueAsString(categoryRepo.findAll()))).listIterator();
					it.hasNext();) {
				assertEquals(categories[it.nextIndex()], 
						((JSONObject) it.next()).get("categoryName").toString());
			}
	...
	public void dbUserDefaultData() throws Exception {
		 ...
		 for(ListIterator<?> it = ((JSONArray) new JSONParser()
					...
					it.hasNext();) {
				assertEquals(users[it.nextIndex()], 
						((JSONObject) it.next()).get("username").toString());
				assertEquals("pass_word", 
						((JSONObject) it.next()).get("password").toString());
			}
	...
	public void dbProductDefaultData() throws Exception{
		...
		for(ListIterator<?> it = ((JSONArray) new JSONParser()
				.parse(new ObjectMapper().writeValueAsString(productRepo.findAll()))).listIterator();
				it.hasNext();) {
	...
	public void compareUserAndCartOwner(){
		...
		String role = u.getRoles().getRole().toString();
		assert(u.getRoles().getRole().toString().contains("CONSUMER"));
		assert(!u.getRoles().getRole().toString().contains("SELLER"));
	}
